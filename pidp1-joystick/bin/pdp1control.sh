...
argc=$#
pidp1="/opt/pidp1-joystick/bin/pdp1"
cd /opt/pidp1-joystick/
...
do_start() {
	...
	if [ "$interface" = "gui" ]; then
		...
		cd /opt/pidp1-joystick/web_pdp1
		nohup go run /opt/pidp1-joystick/web_pdp1/pdpsrv.go > /dev/null 2>&1 &
		cd /opt/pidp1-joystick
...
do_set() {
	...
	case "$2" in
		gui|web|apps)
			...
			temp_file="/opt/pidp1-joystick/bin/pdp1control.tmp"
