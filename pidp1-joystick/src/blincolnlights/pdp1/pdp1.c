...
// ESP32 IoT Joystick functions (implemented in pdp1_joystick.c)
//2025 11 22 i think these declaration are obsolete ...
extern void esp32_send_output(PDP1 *pdp, int device_id);
extern int esp32_read_input(PDP1 *pdp, int device_id);
extern void esp32_poll_input(PDP1 *pdp);
...
iot_pulse(PDP1 *pdp, int pulse, int dev, int nac)
{
	...
	//device 015 is wifi joystick
	case 015:
		if(pulse){
			//receive data from esp32
			//esp32 18 bit data -> pidp1 IO
			pdp1_joy_input(pdp, dev);
		}
		break;
...
if(pdp->pf & 040) printf("	char missed <%o>\n", pdp->tb);
	...
	/* ESP32 IoT Backplane - Poll for input data */
	//esp32_poll_input(pdp);
}
